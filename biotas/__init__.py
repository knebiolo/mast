...
def orionImport(fileName,rxfile,dbName,recName,switch = False, scanTime = None, channels = None, ant_to_rec_dict = None):
    ...
    if 'Type' in columns:
        ...
        telemDat = pd.read_fwf(fileName,colspecs = [(0,12),(13,23),(24,30),(31,35),(36,45),(46,54),(55,60),(61,65)],
                                names = ['Date','Time','Site','Antenna','Freq','Type','Code','Power'],
                                skiprows = 1,
                                dtype = {'Date':str,'Time':str,'Site':np.int32,'Antenna':str,'Freq':str,'Type':str,'Code':str,'Power':np.float64})
        ...
        telemDat = pd.read_fwf(fileName,colspecs = [(0,11),(11,20),(20,26),(26,30),(30,37),(37,42),(42,48)],
                                names = ['Date','Time','Site','Antenna','Freq','Code','Power'],
                                skiprows = 1,
                                dtype = {'Date':str,'Time':str,'Site':str,'Antenna':str,'Freq':str,'Code':str,'Power':str})
    ...
    if len(telemDat) > 0:
        ...
        if len(telemDat) == 0:
            ...
            if ant_to_rec_dict == None:
#               telemDat.drop(['Antenna'], axis = 1, inplace = True)
...
#                conn.executemany('INSERT INTO tblReceiverParameters VALUES (?,?,?,?,?)',recParamLine)
                ...
                for i in ant_to_rec_dict:
                    ...
                    telemDat_sub = telemDat[telemDat.Antenna == str(i)]
...
#                   telemDat_sub.drop(['Antenna'], axis = 1, inplace = True)
...
#       telemDat = telemDat.iloc[:-2]                                                   # remove last two rows, Lotek adds garbage at the end
        ...
        telemDat = pd.read_fwf(fileName,colspecs = [(0,8),(8,23),(23,33),(33,41),(41,56),(56,63),(63,78),(78,90)],names = ['Date','Time','ChannelID','TagID','Antenna','Power','Lat','Long'],skiprows = dataRow)
...
class training_data():
    ...
    def __init__(self,i,site,projectDB,scratchWS):
        ...
        conn = sqlite3.connect(projectDB, timeout=30.0)
        c = conn.cursor()
        sql = "SELECT FreqCode, Epoch, recID, timeStamp, Antenna, Power, Lat, Long, noiseRatio, ScanTime, Channels, RecType FROM tblRaw WHERE FreqCode == '%s' AND tblRaw.recID == '%s';"%(i,site)
...
class classify_data():
    ...
    def __init__(self,i,site,classifyFields,projectDB,scratchWS,training_data,informed_prior = True,training = None, reclass_iter = None):
        ...
        if reclass_iter == None:
            sql = "SELECT FreqCode, Epoch, recID, timeStamp, Antenna, Lat, Long, Power, noiseRatio, ScanTime, Channels, RecType, fileName FROM tblRaw WHERE FreqCode == '%s' AND recID == '%s';"%(i,site)
            self.histDF = pd.read_sql(sql,con = conn,coerce_float  = True)


        else:
            sql = "SELECT FreqCode, Epoch, recID, timeStamp,Antenna, Lat, Long, Power, noiseRatio, ScanTime, Channels, RecType, fileName FROM tblClassify_%s_%s WHERE FreqCode == '%s' AND test == '1';"%(site,reclass_iter-1,i)
            self.histDF = pd.read_sql(sql,con = conn,coerce_float  = True)
...
#            classDF = pd.read_sql("select test, FreqCode,Power,lag,lagDiff,fishCount,conRecLength,consDet,detHist,hitRatio,noiseRatio,seriesHit,timeStamp,Epoch,RowSeconds,recID,RecType,ScanTime from tblClassify_%s_%s"%(site,classify_object.reclass_iter-1),con=conn)
        classDF = pd.read_sql("select test, FreqCode,Lat,Long,Power,noiseRatio, lag,lagDiff,conRecLength_A,consDet_A,detHist_A,hitRatio_A,seriesHit_A,conRecLength_M,consDet_M,detHist_M,hitRatio_M,seriesHit_M,postTrue_A,postTrue_M,timeStamp,Epoch,RowSeconds,recID,RecType,ScanTime from tblClassify_%s_%s"%(site,reclass_iter-1),con=conn)
...
def classDatAppend(site,inputWS,projectDB,reclass_iter = 1):
    ...
    for f in files:
        ...
        #dat.drop(['recID1'],axis = 1,inplace = True)
        dtype = {'FreqCode':'TEXT', 'Epoch': 'INTEGER', 'recID':'TEXT','timeStamp':'TIMESTAMP', 'Antenna':'TEXT', 'Lat':'TEXT','Long':'TEXT', 'Power':'REAL', 'ScanTime':'REAL',
